...
| `retarget_mesh.py` | Blender. Drops unrigged objects, poses the custom mesh's arms onto `rig.json`, then exports the OBJ, its real per-vertex **weights** on rig bone indices, and its **tangent frame** (`_frame.bin`: u, v, normal, tangent, handedness). Replaces `prepare_mesh.py`, which joined the GLB's unrigged icosphere into the body and discarded the armature. |
| `decode_texcoords.py` | Decodes and re-verifies the stride-24 second vertex buffer against the geometry: a candidate normal field is right when it correlates with the normals the triangles actually have. `--search` re-derives the layout from scratch. |

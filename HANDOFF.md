...
**Updated:** 2026-08-24 (**#66 Playbook merged, built, not installed; map mode on with 423 offline maps, not launched**). Earlier: 2026-08-23 (**emotes confirmed working in game** — the Emote Collection is *equipped* in the emote slot, and PR #48 is what made that resolve). Cosmetics: **left-hand roll −25° confirmed in game**. Earlier that day (**left-hand roll −25° shipped, not yet seen** — `037c_34` / `037d_32` / `0698_30`). The v24 hand refit under it is **user-confirmed better** in game. FP arms on gauntlets `(0, 16326)` confirmed in-world. Host Intake tool is live. Race-glove blank still unconfirmed. `--fingers` CLOSED on the chest.
...
**User-confirmed 2026-08-23:** a spawned enemy **tracks you, shoots back, takes damage, damages
you, and drops ammo.** The whole combat loop ships in the client binary and activates on spawn.

Two predictions of mine died here; do not revive either. "Expect props, bullets do nothing" —
wrong. "D2 is Bungie-hosted so the AI may live only on their servers" — wrong, it is in the client.

**The server road is therefore dead for anything you want to see or fight.** The physics server's
actor store, combat kernel, pathfinding, triggers, objectives and timers are real, switched off and
unnecessary: nothing encodes a frame for a server actor, and the client needs none of it.
`ProvingPolicy` stays gated off; it measures that stack honestly and is not a route to gameplay.
The five-layer map below is kept as an accurate description of the server, not as a plan.

**What is missing is encounter design, not combat.** What spawns, where, in what waves, against
what objective.

`stanuwu/Sunrise#58` (merged) populates automatically, two modes on the overlay's **Spawn** page
(`Insert` opens it):

- **roaming ring** (default) — keeps `target` 12 live entities 18-55 m from the player, forgetting
  past 140 m, one placement per 600 ms
- **`useMap`** — places at the destination's own authored positions, respawning a point 45 s after
  its entity dies. `snapToGround` on by default, because an authored height is only as good as the
...
brittle if the build moves off Shadowkeep `86657.20.08.23`.
